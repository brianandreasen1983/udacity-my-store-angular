...
In order to run this application you must have NodeJS installed. If you don't have nodeJS installed please following the instructions on the NodeJS website to ensure it can be run as you will have to install all of the dependencies by running npm install in a terminal window in before running the application.

Download NodeJS Installer: https://nodejs.org/en/download/

after Node JS has been installed you will want to open a terminal window or a command prompt in some operating systems, navigate to the directory of where the project is stored, and run npm install.

The installation of the npm packages can take some time.

Once installed from the same terminal window type in the command ng serve and this will run the application on localhost:4200.

Open a web browser such as Chrome, Firefox, or Edge and type in localhost:4200 or copy/paste it from the readme and put it into the browser it should load the web application as expected.
